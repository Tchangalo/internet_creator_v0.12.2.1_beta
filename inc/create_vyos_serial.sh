...
for r in $(seq $first_router $last_router); do 
	ansible-playbook -i inventories/inventory${provider}.yaml vyos_upgrade_fast.yml "-l p${provider}r${r}v"
done
...
for r in $(seq $first_router $last_router); do 
    ansible-playbook -i inventories/inventory${provider}.yaml vyos_setup_fast.yml "-l p${provider}r${r}v"
done
